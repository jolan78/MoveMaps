...
{
  Vector3
  ModelPosition::transform (const Vector3& pIn) const
  {
    //return(pIn);
  ...
  void
  CMappingEntry::addFilename (char *pName)
  {
  ...
  const std::string
  CMappingEntry::getKeyString () const
  {
  ...
  int
  findPosChar (const char *namebuffer, char pSearch, int pCount)
  {
    int result = -1;
    int pos = 0;
  ...
  CoordModelMapping::getFilenamesForCoordinate (unsigned int pMapId, int xPos, int yPos)
  {
